        ...
        <div className="col-span-2 text-white p-4">
          <div className="space-y-4">
            {flights.map((flightGroup, index) => (
              <div key={index} className="bg-white p-4 shadow-lg rounded-md flex justify-between items-center">
                <div>
                  <span className="text-xs bg-blue-100 text-blue-600 px-2 py-1 rounded">Best</span>
                  {flightGroup.map((flight, flightIndex) => (
                    <div key={flightIndex} className="flex items-center py-2">
                      ...
                      <div className="text-sm">
                        <div className="font-medium text-lg text-blue-600">{flight.departureTime}</div>
                        <div className="text-gray-400 text-xs">{flight.from}</div>
                      </div>
                      <div className="flex-1 text-center">
                        <div className="border-b border-l border-gray-400 mx-2"></div>
                        <div className="text-gray-500 text-xs mr-10 ml-10 mt-2">{flight.direct ? 'EG3146517' : ''}</div>
                      </div>
                      <div className="text-sm">
                        <div className="font-medium text-lg text-blue-600">{flight.arrivalTime}</div>
                        <div className="text-gray-400 text-xs">{flight.to}</div>
                      </div>
                      <div className="text-xs text-gray-500 pl-28">{flight.duration}</div>
                    </div>
                  ))}
                </div>
